...
- 段错误
    - `Segmentation fault`存储器段错误, 也称为权限冲突. 访问的内存超出了系统所给这个程序的内存空间.
    - 段错误就是访问了不可访问的内存, 这个内存区域要么是不存在的, 要么是受到系统保护的.
    - 产生原因:
        - 1.访问不存在的内存地址(`0`地址).
        - 2.访问系统保护的内存地址.
        - 3.访问只读内存地址. 对字符串进行赋值.`strcpy(ptr, "Test");`
        - 4.堆栈溢出.
        - 5.`pthread_create()`失败后仍进行`pthread_join()`
    - 分析和调试手段
        - `catchsegv`: 
        - `gdb`
        - `coredump`: core文件分析.
